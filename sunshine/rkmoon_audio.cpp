...
Pcm open_pcm(const char *device) {
  ...
  int direction = 0;
  // Do not silently record a microphone, resample or change the host audio clock.
  if ((rc = snd_pcm_hw_params_any(pcm.get(), hw)) < 0 ||
      ...
      (rc = snd_pcm_hw_params_set_period_size_near(pcm.get(), hw, &period, &direction)) < 0 ||
      (rc = snd_pcm_hw_params(pcm.get(), hw)) < 0) return {};
...
void audio_capture(safe::mail_t mail, audio::config_t config, void *channel_data) {
  ...
  if (!encoder || error != OPUS_OK ||
      opus_encoder_ctl(encoder.get(), OPUS_SET_BITRATE(96000)) != OPUS_OK ||
  ...
  while (!shutdown->peek()) {
    ...
    if (!pcm && now >= retry_at) {
      ...
      if (pcm) {
        ...
        BOOST_LOG(warning) << "RKMoon HDMI audio unavailable; video/input continue; retrying";
      }
    }
    if (!pcm && !had_source) {
      shutdown->view(200ms);
    ...
    // Pace silence on disconnect; never let a broken audio source stall video or RTP.
    if (!pcm && now < next_packet) {
    ...
    if (pcm) {
      ...
      while (received < frames && !shutdown->peek() && std::chrono::steady_clock::now() < deadline) {
    ...
    if (next_packet < now) next_packet = now; // No burst replay of stale PCM on reinit.
    if (!pcm && next_packet > now) shutdown->view(std::min(20ms, std::chrono::duration_cast<std::chrono::milliseconds>(next_packet - now)));
  }
